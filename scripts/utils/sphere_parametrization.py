...
GAUSSIAN_TO_SHAPE_MODEL_PATH = (
    "out/sphere_parametrization/xyz_tr_reg_080/checkpoint-9999.pth"
)
GAUSSIAN_TO_SPHERE_MODEL_PATH = (
    "out/sphere_parametrization/xyz_sphere/checkpoint-9999.pth"
)
...
def main(args):
    ...
    reconstructed_sphere_points = gaussian_to_sphere_model.sample(
        noise=pullback, num_steps=64
    )

    # Parametrize sphere points with 2 values (u, v) in [0,1] using spherical coords
    r = torch.linalg.norm(reconstructed_sphere_points, dim=-1, keepdim=True).clamp_min(
        1e-8
    )
