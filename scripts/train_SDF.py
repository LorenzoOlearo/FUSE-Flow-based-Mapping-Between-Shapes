...
class MLP(nn.Module):
    def __init__(self, config: MLPConfig) -> None:
        ...
        self.out_dim = (
            config.out_dim if config.out_dim is not None else config.layer_width
        )
        self.num_layers = config.num_layers
        self.layer_width = config.layer_width
        self.skip_connections = config.skip_connections
        self._skip_connections: Set[int] = (
            set(self.skip_connections) if self.skip_connections else set()
        )
        ...
        if config.num_frequencies > 0:
            self.encoding = FourierFeatsEncoding(
                self.in_dim, config.num_frequencies, config.encoding_with_input
            )
        ...
        if self.num_layers == 1:
            ...
            for i in range(self.num_layers - 1):
                if i == 0:
                    ...
                    layers.append(
                        nn.Linear(self.layer_width + self.mlp_in_dim, self.layer_width)
                    )
                else:
            ...
            if j == len(self.layers) - 1:
                if self.out_dim > 1:
                    ...
                    torch.nn.init.normal_(
                        lin.weight,
                        mean=np.sqrt(np.pi) / np.sqrt(lin.in_features),
                        std=0.0001,
                    )
                ...
                torch.nn.init.normal_(
                    lin.weight[:, : self.in_dim],
                    0.0,
                    np.sqrt(2) / np.sqrt(lin.out_features),
                )
            elif self.encoding is not None and j in self.skip_connections:
                torch.nn.init.constant_(lin.bias, 0.0)
                torch.nn.init.normal_(
                    lin.weight, 0.0, np.sqrt(2) / np.sqrt(lin.out_features)
                )
                torch.nn.init.constant_(
                    lin.weight[:, self.in_dim : self.mlp_in_dim], 0.0
                )
            else:
                torch.nn.init.constant_(lin.bias, 0.0)
                torch.nn.init.normal_(
...
class SDF(nn.Module):
    ...
    def distance(
        self, x_in: Float[Tensor, "*batch in_dim"]
    ...
    def sphere_trace(
        self, x_in: Float[Tensor, "*batch 3"], dir: Float[Tensor, "*batch 3"]
    ) -> Float[Tensor, "*batch 3"]:
    ...
    def tangent_plane(
        ...
        I = (
            torch.eye(d, device=x_in.device)
            .view(*([1] * (x_in.dim() - 1)), d, d)
            .expand(*x_in.shape[:-1], -1, -1)
        )
        z = I[..., 2]
    ...
    def sample_zero_level_set(
        ...
        with torch.no_grad():
            while n_samples < num_samples:
                ...
                if igl_sdf is False:
                    print(
                        f" > Sampling using neural SDF... (collected {n_samples}/{num_samples})"
                    )
                    unif = (
                        torch.rand(samples_per_step, 3, device=device)
                        * (bounds[1] - bounds[0])
                        + bounds[0]
                    )
                    ...
                    print(
                        f" > Sampling using IGL SDF... (collected {n_samples}/{num_samples})"
                    )
                    unif = (
                        np.random.rand(samples_per_step, 3).astype(np.float32)
                        * (bounds[1] - bounds[0])
                        + bounds[0]
                    )
...
class NeuralSDF(SDF):
    ...
    def forward(
        ...
        differentiable_grad: bool = False,
...
class MeshData:
    def __init__(self, config: MeshDataConfig):
        ...
        self.geometry = (
            kal.rep.SurfaceMesh(self.verts, self.faces).to_batched().to(config.device)
        )
        ...
        self.domain = torch.tensor([[-1, -1, -1], [1, 1, 1]], dtype=torch.float32).to(
            config.device
        )
...
class MeshDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Dict[str, Float[Tensor, "*batch 3"]]:
        ...
        gauss = (
            surface_pts[:n_gauss]
            + torch.randn(n_gauss, 3, device=self.mesh.device) * self.mesh.noise_scale
        )
...
class ImplicitGeometricRegularization(nn.Module):
    ...
    def forward(
        ...
        eik_loss = self.config.eikonal_error_w * self.eikonal_loss(
            torch.cat([grad_sdf_space, grad_sdf_surf], dim=1)
        )
        normals_loss = self.config.normals_error_w * self.normals_loss(
            grad_sdf_surf, surf_normals
        )
        return {
            "zero_loss": zero_loss,
...
class NormalsLoss(nn.Module):
    ...
    def forward(
        self, grad: Float[Tensor, "*batch 3"], normals: Float[Tensor, "*batch 3"]
    ) -> Float[Tensor, "1"]:
...
class DeformationLoss(nn.Module):
    ...
    def forward(
        ...
        transformed_verts = (rotations @ patch_verts[..., None]).squeeze(
            -1
        ) + translations
        moving_pos = transformed_verts[moving_idx[:, 0], moving_idx[:, 1], :]
        static_pos = transformed_verts[static_idx[:, 0], static_idx[:, 1], :]
        moving_handle_loss = (
            self.handle_loss(moving_pos, moving_gt) if moving_gt.shape[0] > 0 else 0.0
        )
        static_handle_loss = (
            self.handle_loss(static_pos, static_gt) if static_gt.shape[0] > 0 else 0.0
        )
...
def make_volume(device):
    steps = torch.linspace(min_coord, max_coord, resolution, device=device)
        ...
        for batch in dataloader:
            ...
            if epoch % 100 == 0:
                tqdm.write(
                    ...
                    + ", ".join(f"{k}: {v.item():.6f}" for k, v in loss_dict.items())
                )
...
def eval(config: MeshDataConfig):
    ...
    if landmarks_indices is not None and len(landmarks_indices) > 0:
        ...
        for landmark_vertex in landmarks_vertices:
            ...
            landmark_vertex_projected = project_to_surface(
                landmark_vertex, sdf_model, device
            )
            voxel_index = world_to_grid(
                landmark_vertex_projected, min_coord, max_coord, resolution
            )
            print(
                f"> Landmark vertex {landmark_vertex} | Projected in {landmark_vertex_projected} | Mapped to voxel index {voxel_index}"
            )
        ...
        np.save(
            f"{landmarks_path}-landmarks-vertices.npy", landmarks_vertices.cpu().numpy()
        )
    ...
    plot_mesh(
        verts, faces, save_path=f"out/SDFs/{config.file.stem}/SDF-{config.file.stem}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train and evaluate an SDF model on mesh data."
    )
    parser.add_argument(
        "--mesh_path", type=str, help="Path to a single mesh file (.off or .ply)."
    )
    parser.add_argument(
        "--mesh_folder", type=str, help="Folder containing multiple mesh files."
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Train on all mesh files in the specified folder.",
    )
    parser.add_argument(
        "--eval", action="store_true", help="Evaluate the trained SDF model."
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="Only train on meshes with names like 'tr_reg_080' to 'tr_reg_099'.",
    )
    ...
    if args.all:
        ...
        if args.smal == False:
            mesh_files = [
                f
                for f in os.listdir(args.mesh_folder)
                if f.endswith((".off", ".ply"))
                and (
