...
def create_comparison_plots(base_output_dir, experiment_configs, datasets, metrics=['chamfer_distance', 'hausdorff_distance']):
    ...
    for dataset in datasets:
        ...
        for metric_name in metrics:
            ...
            search_pattern = metric_display_names.get(metric_name, metric_name.replace('_', ' '))
            metric_label = "Chamfer Distance" if metric_name == "chamfer_distance" else "Hausdorff Distance"
                ...
                for method in ['FM', 'Geomdist']:
                    ...
                    for dist in ['Gaussian', 'Sphere','Gaussian_Optimized']:
                        ...
                        for config in experiment_configs:
                            if (config['network'] == network and 
                                config['method'] == method and 
                                config['distribution'] == dist):
                                
                                output_dir = f"{base_output_dir}/{base_name}_{config['id']}"
                                ...
                                if os.path.exists(metric_file):
                                    ...
                                    if len(epochs) > 0:
                                        plt.plot(epochs, values, label=f"Distribution: {dist}", 
                                        ...
                                        results_data.append({
                                            ...
                                            'Distribution': dist,
                                            'Metric': metric_name,
                                            'Final Value': values[-1] if len(values) > 0 else None,
                                            'Best Value': min(values) if len(values) > 0 else None
                                        })
                    
                    plt.title(f'{metric_label} - {base_name}\nMethod: {method}, Network: {network}')
                    plt.xlabel('Epochs')
            ...
            for dist in ['Gaussian', 'Sphere', 'Gaussian_Optimized']:
                for method in ['FM', 'Geomdist']:
                    ...
                    for network in ['Network', 'MLP', 'MLP_tiny']:
                        # Find the matching config
                        for config in experiment_configs:
                            if (config['distribution'] == dist and 
                                config['method'] == method and 
                                ...
                                if os.path.exists(metric_file):
                                    ...
                                    epochs, values = parse_metric_file(metric_file, search_pattern)
                                    if len(epochs) > 0:
                                        plt.plot(epochs, values, label=f"Network: {network}", 
                                                marker='o', markersize=3, linestyle='-')
                        ...
                        for config in experiment_configs:
                            if (config['distribution'] == dist and 
                                ...
                                if os.path.exists(metric_file):
                                    ...
                                    epochs, values = parse_metric_file(metric_file, search_pattern)
                                    if len(epochs) > 0:
                                        plt.plot(epochs, values, label=f"Method: {method}", 
                                                marker='o', markersize=3, linestyle='-')
    ...
    if not results_df.empty:
        ...
        results_df.to_csv(os.path.join(plots_dir, 'ablation_detailed_results.csv'), index=False)
        ...
        for metric_name in metrics:
            ...
            dist_method_pivot.to_csv(os.path.join(plots_dir, f'{metric_name}_distribution_method_summary.csv'))
            ...
            network_method_pivot.to_csv(os.path.join(plots_dir, f'{metric_name}_network_method_summary.csv'))
            ...
            if not network_dist_pivot.empty:
                sns.heatmap(network_dist_pivot, annot=True, fmt=".4f", cmap="viridis", 
                           linewidths=.5, cbar_kws={'label': f'Final {metric_label}'})
                
                plt.title(f'Ablation Study Summary - {metric_label}')
                plt.tight_layout()
                plt.savefig(os.path.join(plots_dir, f'{metric_name}_heatmap.png'), dpi=300)
                plt.close()
...
def create_tick_plots(metric_df, metric_name, metric_label, plots_dir):
    ...
    if len(fm_data) > 0 and len(gd_data) > 0:
        ...
        plt.bar(index + bar_width, gd_data, bar_width, label='Geomdist', color='crimson', alpha=0.7)
        
        # Add value annotations
        for i, v in enumerate(fm_data):
            plt.text(i, v + 0.001, f"{v:.4f}", ha='center', va='bottom', fontweight='bold', fontsize=8, rotation=45)
        
        for i, v in enumerate(gd_data):
            plt.text(i + bar_width, v + 0.001, f"{v:.4f}", ha='center', va='bottom', fontweight='bold', fontsize=8, rotation=45)
        ...
        for i in range(len(fm_data)):
            if fm_data[i] > 0 and gd_data[i] > 0:
                ...
                plt.text(i + bar_width/2, max(fm_data[i], gd_data[i]) + 0.003, 
                        f"{pct_diff:.1f}%", ha='center', va='bottom', 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(plots_dir, f'{metric_name}_method_tick_comparison.png'), dpi=300)
        plt.close()
    ...
    # Create tick plot
    ax = sns.barplot(x='Network', y='Final Value', data=network_summary, palette='viridis')
    ...
    plt.ylabel(metric_label)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'{metric_name}_network_tick_comparison.png'), dpi=300)
    plt.close()
    ...
    # Create tick plot
    ax = sns.barplot(x='Distribution', y='Final Value', data=dist_summary, palette='Set2')
    ...
    plt.ylabel(metric_label)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'{metric_name}_distribution_tick_comparison.png'), dpi=300)
    plt.close()
    ...
    grid_data = metric_df.pivot_table(
        values='Final Value',
        index=['Network', 'Distribution'],
        columns=['Method']
    ).reset_index()
    
    # Melt the data for easier plotting
    melted_data = pd.melt(grid_data, 
                          id_vars=['Network', 'Distribution'], 
                          value_vars=['FM', 'Geomdist'],
                          var_name='Method', value_name='Final Value')
    
    # Create the facet grid
    g = sns.catplot(
        data=melted_data,
        kind="bar",
        x="Method", y="Final Value", hue="Distribution",
        col="Network", 
        height=5, aspect=0.8,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'{metric_name}_comprehensive_grid.png'), dpi=300, bbox_inches='tight')
    plt.close()
def create_mean_plots(base_output_dir, experiment_configs, datasets, metrics=['chamfer_distance', 'hausdorff_distance']):
    ...
    for metric_name in metrics:
                ...
                for dist in ['Gaussian', 'Sphere','Gaussian_Optimized']:
                    ...
                    for config in experiment_configs:
                        if (config['method'] == method and 
                            ...
                            if config_id in all_data[metric_name] and len(all_data[metric_name][config_id]['epochs']) > 0:
                                ...
                                if len(all_epochs) > 0:
                                    ...
                                    common_epochs = np.arange(min_epoch, max_epoch + 1, 100)  # Sample every 100 epochs
                                    
                                    # Interpolate all datasets to this common x-axis
                                    interpolated_values = []
                                    
                                    for i, (dataset_epochs, dataset_values) in enumerate(zip(all_epochs, all_values)):
                                        # Only include epochs in the common range
                                        mask = (dataset_epochs >= min_epoch) & (dataset_epochs <= max_epoch)
                                        if np.any(mask):
                                            # Interpolate
                                    ...
                                    if interpolated_values:
                                        ...
                                        linestyle = '-' if network == 'Network' else ('--' if network == 'MLP' else '-.')
                                        ...
                                        line, = plt.plot(common_epochs, mean_values, label=label, 
                                        ...
                                        if method not in best_configs or final_mean < best_configs[method]['value']:
                                            best_configs[method] = {
                                                'value': final_mean,
                                                'config': config_id
                                            }
        ...
        for method, best in best_configs.items():
            plt.plot([], [], ' ', label=f"Best {method}: {best['config']} ({best['value']:.4f})")
        ...
        # Save the plot
        output_file = os.path.join(plots_dir, f'mean_{metric_name}_all_configurations.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
                ...
                if best_config:
                    ...
                    if len(all_epochs) > 0:
                        ...
                        if interpolated_values:
                            ...
                            color = method_colors[method]
                            plt.plot(common_epochs, mean_values, label=label, color=color, linewidth=2)
        ...
        # Save the plot
        output_file = os.path.join(plots_dir, f'mean_{metric_name}_best_configurations.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        ...
        # Plot the bar chart
        ax = sns.barplot(x='Configuration', y='Final Mean', hue='Method', data=final_data, palette=method_colors)
        
        # Add value annotations
        for i, v in enumerate(final_data['Final Mean']):
            ax.text(i, v + 0.001, f"{v:.4f}", ha='center', va='bottom', fontweight='bold', fontsize=8, rotation=45)
        ...
        plt.close()
def main():
    parser = argparse.ArgumentParser(description='Run ablation study across multiple configurations')
    parser.add_argument('--base_output_dir', default='ablation', help='Base directory for outputs')
    ...
    parser.add_argument('--data_files', nargs='+', default=[], help='Specific data files to process')
    ...
    for data_file in data_files:
        ...
        for config in experiment_configs:
            ...
            cmd = [
                ...
                "--num_points_train", str(args.num_points),
