...
def get_targets(overwrite, target) -> List[str]:
    ...
    if target is not None:
        target_files = [f for f in os.listdir(TOSCA_DIR) if f.startswith(target) and f.endswith('.ply')]
...
def main(args):
    ...
    for target in targets:
        ...
        features_path = Path(working_dir, 'data', 'TOSCA_features_pca_20', f"{target}_features.npy")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target' , type=str, default=None, help="Target to process (cat, centaur, david, dog, gorilla, horse, michael, victoria, wolf), if None, process all targets")
