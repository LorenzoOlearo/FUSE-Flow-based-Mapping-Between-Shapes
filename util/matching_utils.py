...
import ot
import trimesh
from geomfum.convert import (FmFromP2pConverter, GPUEuclideanNeighborFinder,
                             NamFromP2pConverter, P2pFromFmConverter,
                             P2pFromNamConverter)
...
from geomfum.descriptor.spectral import (LandmarkWaveKernelSignature,
                                         WaveKernelSignature)
from geomfum.functional_map import (FactorSum, LBCommutativityEnforcing,
...
def compute_p2p_with_flows_composition_hungarian(
    ...
    elapsed_time = end_time - start_time
    tqdm.write(
        f">>>> compute_p2p_with_flows_composition_hungarian elapsed {elapsed_time:.4f}s"
    )
    return p2p, elapsed_time
...
def compute_p2p_with_flows_composition_lapjv(
    ...
    elapsed_time = end_time - start_time
    tqdm.write(
        f">>>> compute_p2p_with_flows_composition_lapjv elapsed {elapsed_time:.4f}s"
    )
    return p2p, elapsed_time
...
def compute_p2p_with_inverted_flows_in_gauss(
    ...
    elapsed_time = end_time - start_time
    tqdm.write(
        f">>>> compute_p2p_with_inverted_flows_in_gauss elapsed {elapsed_time:.4f}s"
    )
...
def compute_p2p_with_flows_composition_zoomout(
    ...
    elapsed_time = end_time - start_time
    tqdm.write(
        f">>>> compute_p2p_with_flows_composition_zoomout elapsed {elapsed_time:.4f}s"
    )
...
def compute_p2p_with_knn_zoomout(
...
_ndp_dir = os.path.normpath(
    os.path.join(
        _base_dir, "../", "Shape_Matching_Baseline_wrapper", "DeformationPyramid"
    )
)
