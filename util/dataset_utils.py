...
def get_shrec20_landmarks(models_path, gts_path, plot):
    ...
    for model in model_names:
        ...
        if missing:
            ...
            raise RuntimeError(f"Model {model} missing common IDs: {missing[:10]} (showing up to 10)")
        ...
        if plot == True:
            ...
            fig = go.Figure(data=[
                ...
                go.Scatter3d(
                    ...
                    text=[str(common_ids[i]) for i in range(len(common_ids))],      # index in the common_ids list
    ...
    common_landmarks_df = pd.DataFrame(
...
if __name__ == "__main__":
    ...
    selected_landmarks = common_landmarks_df[['Model',
                                              landmark_head_id,
    ...
    output_selected_csv_path = config["matching_config"]["SHREC20"]["common_landmarks_path"]
