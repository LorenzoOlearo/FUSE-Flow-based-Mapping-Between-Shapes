...
class PointEmbed(nn.Module):
    ...
    def forward(self, input):
        ...
        if others is None:
            embed = self.mlp(
                torch.cat([self.embed(input, self.basis), input], dim=1)
            )  # N x C
        else:
            embed = self.mlp(
...
class Network(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for (x_proj_pre, x_proj_post, emb_linear), emb_gain in zip(
            self.layers, self.gains
        ):
...
class RandomFourierFeatures(nn.Module):
    def __init__(self, input_dim, output_dim, scale=1.0):
        ...
        self.scale = scale
        self.B = nn.Parameter(
            self.scale * torch.randn(self.input_dim, self.output_dim // 2),
            requires_grad=False,
        )
...
class FourierFeatsEncoding(nn.Module):
    ...
    def __init__(
        self, in_dim: int, num_frequencies: int, include_input: bool = False
    ) -> None:
